 ...
 */
class CheckCreateDestDir
{
    public function run(string $dir, int $dir_permissions, bool $create, object $err, string $input)
    {
        if (file_exists($dir)) {
            if (is_dir($dir)) {
                if (is_writable($dir)) {
                    ...
                    if (chmod($dir, $dir_permissions)) {
                        ...
                        $err->set($input, ['errors' => 'ERROR! Dir "' . $dir . '": ' . $err->errors[2] . '<br />']);
                        return false;
                    }
                }
            } else {
                $err->set($input, ['errors' => 'ERROR! Dir "' . $dir . '": ' . $err->errors[3] . '<br />']);
                return false;
            }
        } else {
            ...
            if ($create) {
                if (mkdir($dir, $dir_permissions, true)) {
                    ...
                    $err->set($input, ['errors' => 'ERROR! Dir "' . $dir . '": ' . $err->errors[4] . '<br />']);
                    return false;
                }
            } else {
                $err->set($input, ['errors' => 'ERROR! Dir "' . $dir . '": ' . $err->errors[5] . '<br />']);
                return false;
            }
        }
    }
